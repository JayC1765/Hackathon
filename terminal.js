...
return union([arr1, arr2, arr3]) //-> should log: [5, 10, 15, 88, 1, 7, 100]`, JSON.stringify([5, 10, 15, 88, 1, 7, 100])],
[`// Construct a function objOfMatches that accepts two arrays and a callback. objOfMatches will build an object and return it. To build the object, objOfMatches will test each element of the first array using the callback to see if the output matches the corresponding element (by index) of the second array. If there is a match, the element from the first array becomes a key in an object, and the element from the second array becomes the corresponding value.

const arr1 = ['hi', 'howdy', 'bye', 'later', 'hello'];
const arr2 = ['HI', 'Howdy', 'BYE', 'later', 'HELLO'];
function uppercaser(str) { return str.toUpperCase(); }
return objOfMatches(arr1, arr2, uppercaser); //-> should log: { hi: 'HI', bye: 'BYE', hello: 'HELLO' }`, JSON.stringify({ hi: 'HI', bye: 'BYE', hello: 'HELLO' })],
...
return arrToObj(arrOfStrings, capitalize); //-> should log: { beer: 'BEER', glue: 'GLUE' }`, JSON.stringify({ beer: 'BEER', glue: 'GLUE' })],
...
return majority([2, 3, 4, 5], isOdd); //-> should log: false`, JSON.stringify(false)],
[`// Create a function prioritize that accepts an array and a callback. The callback will return either true or false. prioritize will iterate through the array and perform the callback on each element, and return a new array, where all the elements that yielded a return value of true come first in the array, and the rest of the elements come second.

function startsWithS(str) { return str[0].toLowerCase() === 's'; }
const tvShows = ['curb', 'rickandmorty', 'seinfeld', 'sunny', 'friends']
return prioritize(tvShows, startsWithS); //-> should log: ['seinfeld', 'sunny', 'curb', 'rickandmorty', 'friends']`, JSON.stringify(['seinfeld', 'sunny', 'curb', 'rickandmorty', 'friends'])],
[`// Create a function groupBy that accepts an array and a callback, and returns an object. groupBy will iterate through the array and perform the callback on each element. Each return value from the callback will be saved as a key on the object. The value associated with each key will be an array consisting of all the elements that resulted in that return value when passed into the callback.

const decimals = [1.3, 2.1, 2.4];
const floored = function(num) { return Math.floor(num); };
return groupBy(decimals, floored); //-> should log: { 1: [1.3], 2: [2.1, 2.4] }`, JSON.stringify({ 1: [1, 3], 2: [2.1, 2.4] })],
...
return goodKeys(sunny, startsWithBird); //-> should log: ['charlie', 'dee']`, JSON.stringify(['charlie', 'dee'])],
...
return getDay(); //-> should log: 'Fri'`, JSON.stringify('Fri')],
...
return changeScene('The quick, brown fox jumps over the lazy dogs.'); //-> should log: 'The slow, brown fox jumps over the lazy cats.'`, 'The slow, brown fox jumps over the lazy cats.'],
[`// Get the length of an array using recursion without accessing its length property.

return getLength([1]); //-> 1
return getLength([1, 2]); //-> 2
return getLength([1, 2, 3, 4, 5]); //-> 5
return getLength([]); //-> 0`, JSON.stringify(0)],
...
return pow(2, 4); //-> 16
return pow(3, 5); //-> 243`, JSON.stringify(243)],
];

document.addEventListener('DOMContentLoaded', () => {
  const terminalContainer = document.querySelector('form');
  const terminal = document.querySelector('#terminal');
  let problem = codingQuestions[Math.floor(Math.random() * codingQuestions.length)];
  terminal.value = problem[0];
  terminalContainer.addEventListener('reset', (event) => {
    event.preventDefault();
    problem = codingQuestions[Math.floor(Math.random() * codingQuestions.length)];
    terminal.value = problem[0];
  ...
  terminalContainer.addEventListener('submit', (event) => {
    ...
    if (document.querySelector('#console').value === problem[1]) {
